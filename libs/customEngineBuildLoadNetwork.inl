...
template <typename T>
bool customEngine<T>::buildLoadNetwork(std::string onnxModelPath, const std::array<float, 1> &subVals, const std::array<float, 1> &divVals,
                                 bool normalize) {
...
template <typename T>
bool customEngine<T>::loadNetwork(std::string trtModelPath, const std::array<float, 1> &subVals, const std::array<float, 1> &divVals,
                            bool normalize) {
    m_subVals = subVals;
    m_divVals = divVals;
...
template <typename T>
bool customEngine<T>::build(std::string onnxModelPath, const std::array<float, 1> &subVals, const std::array<float, 1> &divVals, bool normalize) {
    ...
    if (m_options.precision == Precision::FP16) {
        ...
        m_calibrator = std::make_unique<customInt8EntropyCalibrator2>(m_options.calibrationBatchSize, inputDims.d[3], inputDims.d[2],

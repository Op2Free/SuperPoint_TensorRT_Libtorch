...
class customInt8EntropyCalibrator2 : public nvinfer1::IInt8EntropyCalibrator2 {
public:
    customInt8EntropyCalibrator2(int32_t batchSize, int32_t inputW, int32_t inputH, const std::string &calibDataDirPath,
                           const std::string &calibTableName, const std::string &inputBlobName,
                           const std::array<float, 1> &subVals = {0.f}, const std::array<float, 1> &divVals = {1.f},
                           bool normalize = true, bool readCache = true);
    virtual ~customInt8EntropyCalibrator2();
...
private:
    ...
    const std::array<float, 1> m_subVals;
    const std::array<float, 1> m_divVals;
